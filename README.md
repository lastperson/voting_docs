...
## Using Ambisafe Signing Provider with Web3 in Browser

### Initializing the provider:

```javascript
  var wallet = Wallet.fromAmbi('uuid');
  //account = wallet.getAddress();
  var web3Provider = new AmbiProvider({
...
var tx = {
  data: Poll.getData({data: pollBytes})
};
...
var tx = {
  to: pollAddress,
  data: fun.toPayload(data).data
};
var tx = web3.eth.sendTransaction(tx, function(error, transaction) {
  if(!error) {
    console.log('tx sent: ' + transaction.hash);
  } else {
    console.error(error);
